...
2. **标题识别**: 
   ...
   - 自动识别并分离 `**文本**：内容` 格式，标题独立成行，内容作为正文
   - 标题独立成行，不会被合并到其他段落
...
- `Pt(6)`: 段落间距（当前设置为6pt）
- `1.5`: 行距倍数（当前设置为1.5倍）
- `is_title()`: 标题识别规则（支持emoji图标标题、Markdown加粗格式**文本**、**文本**：内容格式，可添加更多标题关键词）
- `extract_bold_title()`: 提取Markdown加粗格式的标题和内容
