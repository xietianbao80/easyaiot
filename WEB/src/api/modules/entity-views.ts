 ...
 * @description: 告警列表
 */
export const getAlarm = ({ module, id, params }) => {
  defHttp.setHeader({ 'X-Authorization': 'Bearer ' + localStorage.getItem('jwt_token') });
  return defHttp.post(
    {
      url: Api.Alarm + `${module}/${id}`,
      params,
    },
    { isTransformResponse: false },
  );
 ...
 * @description: 告警列表
 */
export const getEvents = ({ module, id, params }) => {
  defHttp.setHeader({ 'X-Authorization': 'Bearer ' + localStorage.getItem('jwt_token') });
  return defHttp.get(
    {
      url: Api.Events + `${module}/${id}`,
      params,
    },
    { isTransformResponse: false },
  );
