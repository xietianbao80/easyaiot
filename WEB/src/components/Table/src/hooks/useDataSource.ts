  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      if(params.customApi != null || params.customApi != undefined) {
        rawDataSourceRef.value = await params?.customApi(params);
      } else {
      ...
      let resultItems: Recordable[] = isArrayResult ? rawDataSourceRef.value : get(rawDataSourceRef.value, listField)
      const resultTotal: number = isArrayResult ? rawDataSourceRef.value.length : get(rawDataSourceRef.value, totalField)
